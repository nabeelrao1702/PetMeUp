...
import { GetAllMessages } from "../../../redux/actions/UserActions";
...
const ChatMenu = () => {
  ...
  const { messages, loading } = useSelector((state) => state?.userData);
  ...
  if (messages && messages?.length > 0) {
    sortedData = messages
      .filter((message) => message?.receiver_user?._id === id)
      .sort((a, b) => {
