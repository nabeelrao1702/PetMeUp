...
const Account = () => {
  const freeuserId = useSelector((state) => state?.doctorData?.freeuserInfo?._id);
  const freeuserInfo = useSelector((state) => state?.doctorData?.freeuserInfo);
  ...
  const { firstName, lastName, email } = freeuserInfo;
  ...
  const onClickDoctorDelete = async (freeuserId) => {
    dispatch(DeleteDoctorAccount(freeuserId, navigate, toast));
  };

  const onSubmitDoctor = async (formData) => {
    setModal(false);
    dispatch(UpdateDoctorAccount(freeuserId, formData, navigate, toast));
  };
  ...
  const options = [
    ...
    {
      value: "Unable to receive pet profile requests",
      text: "Unable to receive pet profile requests",
    },
    {
      value: "Unhappy with PetmeUp policies",
      text: "Unhappy with PetmeUp policies",
    },
    {
      value: "I can not find what i need on PetmeUp",
      text: "I can not find what i need on PetmeUp",
    },
          ...
          <div className="w-full px-4 mx-auto ">
            ...
            >
              <Form className=" space-y-4">
                ...
                <button
                  className="btn w-80 btn-primary btn-regular bg-orange-600 hover:bg-orange-700"
                  onClick={handleModal}
                  type="button"
                >
                  Save Changes
          ...
          <div className="item">
            <div className="flex flex-col flex-nowrap text-sm font-medium text-gray-700 space-y-2">
              ...
              <li className="item text-xs font-medium text-gray-500">
                Your profile won't be shown on PetmeUp anymore.
              </li>
              <li className="item text-xs font-medium text-gray-500">
                Active connections will be cancelled.
              </li>
            </div>
          </div>
        </section>
          ...
          <div className="item">
            <div className="flex flex-col text-sm font-medium text-gray-700 space-y-16">
              <select
                ...
                className="w-full lg:w-80 text-sm font-medium text-gray-700 block border border-gray-300 bg-white rounded-md shadow-sm focus:outline-none focus:ring-orange-500 focus:border-orange-500 "
              >
                {options.map((option) => (
              ...
              <button
                ...
                className="btn btn-primary btn-regular bg-orange-600 hover:bg-orange-700"
              >
                Delete Account
              </button>
              {deleteModal && (
                  ...
                  <div className="fixed inset-0 z-10 overflow-y-auto">
                    <div className="flex min-h-full items-end justify-center p-4 text-center sm:items-center sm:p-0">
                      <div className="relative transform overflow-hidden rounded-lg bg-white text-left shadow-xl transition-all sm:my-8 sm:w-full sm:max-w-lg">
                        ...
                        <div className="bg-gray-50 px-4 py-3 sm:flex sm:flex-row-reverse sm:px-6">
                          <button
                            onClick={() => onClickDoctorDelete(freeuserId)}
                            className="inline-flex w-full justify-center rounded-md border border-transparent bg-red-600 px-4 py-2 text-base font-medium text-white shadow-sm hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2 sm:ml-3 sm:w-auto sm:text-sm"
                          >
                            I am Sure!
                          </button>
