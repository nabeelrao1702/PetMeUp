...
const OngoingJob = () => {
  ...
  const [days, setDays] = useState(
    sessionStorage.getItem(`jobDaysDoctor-${id}`) || duration
  );
  const [hours, setHours] = useState(
    sessionStorage.getItem(`jobHoursDoctor-${id}`) || 0
  );
  const [minutes, setMinutes] = useState(
    sessionStorage.getItem(`jobMinutesDoctor-${id}`) || 0
  );
  const [seconds, setSeconds] = useState(
    sessionStorage.getItem(`jobSecondsDoctor-${id}`) || 0
  );

  useEffect(() => {
    sessionStorage.setItem(`jobDaysDoctor-${id}`, days);
    sessionStorage.setItem(`jobHoursDoctor-${id}`, hours);
    sessionStorage.setItem(`jobMinutesDoctor-${id}`, minutes);
    sessionStorage.setItem(`jobSecondsDoctor-${id}`, seconds);
                      ...
                      (skill, index) => (
                        ...
                        >
                          <div className="item ">
                            <div className="item w-full h-auto flex-grow  ">
                              <div className="flex flex-row flex-wrap space-x-2 w-full ">
                                <div className=" px-3 py-1 text-sm font-thin text-white bg-fuchsia-600  rounded-md ">
                                  <span>{skill}</span>
                                </div>
                              </div>
                            </div>
              ...
              <div className="flex flex-col flex-wrap w-auto flex-grow px-4 my-4 space-y-3">
                ...
                <div className="item">
                  <div className="flex px-2 py-2  text-sm font-thin text-gray-600 bg-blue-300  rounded-md ">
                    ...
                    <span className="-mt-1">
                      Est.Check-Up Fee ${singleJob?.jobOrderType?.job?.budget}
                    </span>
                  </div>
                </div>
                <div className="item">
