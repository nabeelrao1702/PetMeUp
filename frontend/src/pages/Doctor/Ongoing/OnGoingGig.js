...
const OngoingGig = () => {
  ...
  const freeuserId = localStorage.getItem("authID");
  ...
  const [days, setDays] = useState(
    sessionStorage.getItem(`gigDaysDoctor-${id}`) || duration
  );
  const [hours, setHours] = useState(
    sessionStorage.getItem(`gigHoursDoctor-${id}`) || 0
  );
  const [minutes, setMinutes] = useState(
    sessionStorage.getItem(`gigMinutesDoctor-${id}`) || 0
  );
  const [seconds, setSeconds] = useState(
    sessionStorage.getItem(`gigSecondsDoctor-${id}`) || 0
  );

  useEffect(() => {
    sessionStorage.setItem(`gigDaysDoctor-${id}`, days);
    sessionStorage.setItem(`gigHoursDoctor-${id}`, hours);
    sessionStorage.setItem(`gigMinutesDoctor-${id}`, minutes);
    sessionStorage.setItem(`gigSecondsDoctor-${id}`, seconds);
          ...
          <div className="flex flex-wrap w-auto mx-6 lg:mx-20 my-8">
                    ...
                    {gigOrder?.gigOrderType?.skills?.map((skill, index) => (
                      ...
                      >
                        <div className="item ">
                          <div className="item w-full h-auto flex-grow  ">
                            <div className="flex flex-row flex-wrap space-x-2 w-full ">
                              <div className=" px-3 py-1 text-sm font-thin text-white bg-fuchsia-500  rounded-md ">
                                <span>{skill}</span>
                              </div>
                            </div>
                          </div>
            ...
            <div className=" item w-full lg:w-1/4 h-auto border-3 border lg:rounded-r-md border-l-0 border-gray-300">
              <div className="flex flex-col flex-wrap w-auto flex-grow px-4 my-4 space-y-3">
                <div className="item">
                  <div className="flex px-2 py-2 text-sm font-thin text-gray-700 bg-blue-300 rounded-md ">
                    <svg
                    ...
                    <span className="-mt-1">
                      Visit In: {gigOrder?.duration} Days
                    </span>
                  </div>
                </div>
                <div className="item">
                  <div className="flex px-2 py-2 text-sm font-thin text-gray-600 bg-blue-300  rounded-md ">
                    <svg
                    ...
                    <span className="-mt-1">
                      User budget: {getPackage(gigOrder?.duration, gigOrder?.price)}
                    </span>
                  </div>
                </div>
                <div className="item">
                ...
                <div className="item">
                  <button
                    ...
                    className="w-full h-12 px-12 text-sm font-medium text-white transition-colors duration-150 bg-red-600 border border-transparent rounded-md active:bg-red-600 hover:bg-red-700 focus:outline-none focus:shadow-outline-orange"
                  >
                  ...
                  <div className="bg-white text-gray-700  py-3 rounded text-center">
                    <div className="flex flex-col mt-4 justify-center items-center">
                      ...
                      <span className="font-bold ">
                        You Have Already Reviewed The User
                      </span>
                    </div>
                  </div>
                )}
