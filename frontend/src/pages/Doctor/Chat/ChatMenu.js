...
import { GetAllMessages } from "../../../redux/actions/DoctorActions";
...
const ChatMenu = () => {
  ...
  const { messages, loading } = useSelector((state) => state?.doctorData);
  ...
  if (messages && messages?.length > 0) {
    sortedData = messages
      .filter((message) => message?.receiver_doctor?._id === id)
      .sort((a, b) => {
  ...
  return (
    <div>
        ...
        >
                  ...
                  <li key={index} className="flex">
                    <div className="inline-flex cursor-pointer items-center border-b -mb-2 w-full px-2 py-2 text-sm transition-colors duration-150 hover:bg-gray-100 hover:text-gray-800 ">
                      <img
                        ...
                        src={
                          message?.sender_doctor?.photo ||
                          message?.sender_user?.photo ||
                      ...
                      <p className="text-gray-600 text-sm mx-2">
                        <span className="font-bold" href="#">
                          {message?.sender_doctor?.firstName ||
                            message?.sender_user?.firstName ||
                            message?.sender_admin?.username}
                        </span>{" "}
                        <span className="font-bold" href="#">
                          {message?.sender_doctor?.lastName ||
                            message?.sender_user?.lastName}
                        </span>{" "}
          ...
          <li className="flex">
            <Link
              className="block bg-orange-600 text-white border border rounded-b-md w-full text-center font-bold py-2 "
              to="/doctor/messages"
            >
              See all messages
            </Link>
